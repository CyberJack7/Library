...
function getServicesById(category) {
  ...
  let text = document
    .getElementById("show_categories")
    .getElementsByTagName("option")[0];
  let amount_checked_categories = parseInt(text.innerHTML.match(/\d+/), 10);
  if (category.checked == true) {
    text.innerHTML =
      "Выбрано категорий услуг: " + (amount_checked_categories + 1);
  } else {
    text.innerHTML =
      "Выбрано категорий услуг: " + (amount_checked_categories - 1);
...
function searchAutoservices(search_button) {
  ...
  let autoservice_auto_id =
    document.getElementById("autos").options[
      document.getElementById("autos").selectedIndex
    ].value; //нужный автомобиль
  let autoservice_categories_input = document
    .getElementById("categories")
    .getElementsByTagName("input");
  ...
  for (let i = 0; i < amount_categories_input; i++) {
    if (autoservice_categories_input[i].checked == true) {
      autoservice_categories.push(
        parseInt(autoservice_categories_input[i].id.match(/\d+/), 10)
      );
    }
  }
  let autoservice_services_input = document
    .getElementById("services")
    .getElementsByTagName("input");
  ...
  for (let i = 0; i < amount_services_input; i++) {
    if (autoservice_services_input[i].checked == true) {
      autoservice_services.push(
        parseInt(autoservice_services_input[i].id.match(/\d+/), 10)
      );
    }
  }
  ...
  $.post(
    ...
    function (data) {
      ...
      if (
        document
          .getElementById("search_autoservices")
          .getElementsByClassName("plate")[0]
      ) {
        getAutoserviceInfo(
          document
            .getElementById("search_autoservices")
        ...
        cancelApplication();
        document.getElementsByClassName("show_autoservice")[0].style.display =
          "none";
      }
    }
  );
...
function setServiceInfo(ar_service_info) {
  ...
  let elem_a = document
    .getElementById("service_info")
    .getElementsByTagName("a")[0];
  elem_a.href = ar_service_info["certification"];
  elem_a.innerHTML = ar_service_info["certification"].substring(
    ar_service_info["certification"].indexOf("-") + 1
  );
...
function getServiceInfo(select) {
  ...
  let autoservice_id =
    document.getElementsByClassName("show_autoservice")[0].id;
  let json_data = JSON.stringify({
...
function createApplication(btn) {
  ...
  let auto_id =
    document.getElementById("autos").options[
      document.getElementById("autos").selectedIndex
    ].value; //id выбранного автомобиля
  let autoservice_id =
    current_autoservice.getElementsByClassName("show_autoservice")[0].id; //id выбранного СЦ
  let services_inputs = document
    .getElementById("services")
    .getElementsByTagName("input");
...
function setPrice(check) {
  ...
  let autoservice_id = document
    .getElementById("current_autoservice")
    .getElementsByClassName("show_autoservice")[0].id; //id выбранного СЦ
  let json_data = JSON.stringify({
    autoservice_id: autoservice_id,
    service_id: service_id,
  });
  $.post(
    ...
    { get_price: json_data },
    function (data) {
      let cur_price_area = document
        .getElementById("price")
        .getElementsByTagName("b")[0];
      let cur_price = Number(
        parseInt(cur_price_area.innerHTML.match(/\d+/), 10)
      );
      let cur_amount_services_area = document
        .getElementById("show_application_services")
        .getElementsByTagName("option")[0];
      let cur_amount_services = Number(
        parseInt(cur_amount_services_area.innerHTML.match(/\d+/), 10)
      );
      ...
      cur_amount_services_area.innerHTML =
        "Выбрано услуг: " + String(cur_amount_services);
    }
  );
}
