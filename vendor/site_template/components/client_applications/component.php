...
if (isset($_POST['submit_pay_application'])) {
    ...
    if (!empty($user_requisites)) {
        ...
        if ($application_price > $user_requisites['fund_balance']) {
            ...
            if (!empty($application_pay['cardholder_name']) && mb_strtolower($application_pay['cardholder_name']) != mb_strtolower($user_requisites['cardholder_name']) ||
            $application_pay['card_number'] != $user_requisites['card_number'] || $application_pay['validity_date'] != $user_requisites['validity_date'] || $application_pay['cvc_cvv'] != $user_requisites['cvc_cvv']) {
